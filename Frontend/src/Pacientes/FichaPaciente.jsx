...
export default function FichaPaciente() {
  ...
  useEffect(() => {
    const loadTipos = async () => {
      try {
        ...
        if (!headers) return;
        const res = await axios.get("http://localhost:5000/api/examenes", {
          headers,
        });
  ...
  console.debug(
    "[FichaPaciente] initial resolved id from location/query/state:",
    id
  );
    ...
    const fetchPaciente = async () => {
      ...
      try {
        ...
        try {
          ...
          if (headers2) {
            const afterId =
              (data && (data.id_paciente || data.id || data.idPaciente)) || id;
            console.debug(
              "[FichaPaciente] resolved id after fetching paciente:",
              afterId
            );
            if (afterId) {
              // fetch examenes realizados (nuevo endpoint)
              try {
                const reExReal = await axios.get(
                  `http://localhost:5000/api/examenes_realizados`,
                  { headers: headers2 }
                );
                const allExReal = reExReal.data || [];
                // filtrar por paciente
                const filtered = allExReal.filter(
                  (ex) => String(ex.id_paciente) === String(afterId)
                );
                console.debug(
                  "[FichaPaciente] fetched examenes_realizados for patient:",
                  filtered
                );
              ...
              try {
                const re = await fetch(
                  `http://localhost:5000/api/pacientes/${afterId}/examenes`,
                  { headers: headers2 }
                );
                if (re.ok) {
                  const ee = await re.json();
                  console.debug(
                    "[FichaPaciente] fetched examenes (post-paciente):",
                    ee
                  );
              ...
              try {
                const rc = await fetch(
                  `http://localhost:5000/api/pacientes/${afterId}/citas`,
                  { headers: headers2 }
                );
                if (rc.ok) {
                  const cc = await rc.json();
                  console.debug(
                    "[FichaPaciente] fetched citas (post-paciente):",
                    cc
                  );
      ...
      try {
        ...
        try {
          ...
          const filtered = all.filter(
            (ex) => String(ex.id_paciente) === String(id)
          );
          setExamenesRealizados(filtered);
        } catch (e) {
          setExamenesRealizados([]);
        ...
        try {
          const r1 = await fetch(
            `http://localhost:5000/api/pacientes/${id}/examenes`,
            { headers }
          );
          if (r1.ok) {
        ...
        try {
          const r2 = await fetch(
            `http://localhost:5000/api/pacientes/${id}/citas`,
            { headers }
          );
          if (r2.ok) {
  ...
  const refreshCitas = async () => {
    ...
    try {
      ...
      const r = await fetch(`http://localhost:5000/api/pacientes/${id}/citas`, {
        headers,
      });
      if (r.ok) {
  ...
  const openEditModal = () => {
    ...
    setEditForm({
      ...
      sexo: paciente.sexo
        ? String(paciente.sexo).toUpperCase()
        : paciente.sexo ?? "",
      dui: paciente.dui ?? "",
  ...
  const editEdadLocal = (() => {
    ...
    if (editForm.fecha_nacimiento) {
      ...
      editForm.edad !== "" &&
      editForm.edad !== null &&
      typeof editForm.edad !== "undefined"
    ) {
  ...
  const validateEditBeforeSubmit = () => {
    ...
    if (
      !editForm.sexo ||
      !/^[MFOU]$/.test(String(editForm.sexo).toUpperCase())
    ) {
  ...
  const submitEdit = async (ev) => {
    ...
    try {
      ...
      try {
        ...
        const r = await fetch(`http://localhost:5000/api/pacientes/${id}`, {
          headers: headers2,
        });
        if (r.ok) {
  ...
  const handleDeletePaciente = async () => {
    ...
    try {
      ...
      const resolvedId =
        (paciente && (paciente.id_paciente || paciente.id)) || id;
      if (!resolvedId)
        throw new Error("ID de paciente desconocido para eliminar");
      ...
      console.debug(
        "[FichaPaciente] Deleting paciente via REST id=",
        resolvedId
      );
      let res = await fetch(
        `http://localhost:5000/api/pacientes/${resolvedId}`,
        {
          method: "DELETE",
          headers: { ...authHeader },
        }
      ...
      if (
        ...
        console.debug(
          "[FichaPaciente] Primary DELETE failed, trying DELETE with JSON body fallback"
        );
        try {
          res = await fetch(
            `http://localhost:5000/api/pacientes/${resolvedId}`,
      ...
      if (
        ...
        try {
          res = await fetch(
            `http://localhost:5000/api/pacientes?id=${resolvedId}`,
            {
              method: "DELETE",
              headers: { ...authHeader },
            }
      ...
      Swal.fire(
        "Error",
        err.message || "No se pudo eliminar el paciente.",
        "error"
      );
    } finally {
      setLoading(false);
  ...
  const exportExamen = async (idExamen) => {
    try {
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: "No se pudo exportar el examen.",
      });
    }
  };

  // Imprimir examen realizado (usa flujo de PDF del backend para mantener fidelidad al template)
  const printExamen = async (idExamen) => {
    // Abrir ventana de forma síncrona para evitar bloqueo
    const printWindow = window.open("", "_blank", "width=900,height=800");
    if (!printWindow) {
      Swal.fire({
        icon: "warning",
        title: "Ventana bloqueada",
        text: "Permita ventanas emergentes para imprimir.",
      });
      return;
    }
    ...
    try {
      printWindow.document.write(
        '<html><head><title>Preparando impresión...</title></head><body><p style="font-family: Arial, Helvetica, sans-serif;">Preparando documento para imprimir. Por favor espere...</p></body></html>'
      );
    ...
    try {
      const res = await axios.get(
        `http://localhost:5000/api/examenes_realizados/${idExamen}/export?asPdf=1`,
        {
          responseType: "blob",
          headers: getAuthHeaders(),
        }
      ...
      console.error("Error al pedir PDF para imprimir:", err);
      try {
        printWindow.close();
      } catch (_) {}
      Swal.fire({
        icon: "error",
        title: "Error",
        text: "No se pudo preparar el PDF para imprimir. Por favor, descargue el .docx y verifique que LibreOffice esté instalado en el servidor.",
      });
    }
  };
  ...
  const getExamenNombre = (idExamen) => {
    const ex = tiposExamenes.find(
      (t) => Number(t.id_examen) === Number(idExamen)
    );
      ...
      <div className="d-flex">
        ...
        >
          <div className="container-fluid">
            <button
              className="btn"
              style={{ backgroundColor: "#FF8789", color: "#fff" }}
              onClick={() => navigate(-1)}
            >
              Volver
            </button>
            <div className="d-flex justify-content-between align-items-center mb-4 mt-3">
              ...
              <div className="d-flex gap-2">
                <button
                  ...
                  onClick={() =>
                    navigate("/citas", { state: { preselectPacienteId: id } })
                  }
                >
                  Crear Cita
                </button>
                <button
                  ...
                  style={{ backgroundColor: "#00C2CC", color: "#fff" }}
                  onClick={() =>
                    navigate("/realizar-examen", {
                      state: { preselectedPacienteId: id },
                    })
                  }
                >
                  Registrar Examen
            ...
            <p className="text-muted mb-4">
              Información detallada e historial médico
            </p>
                  ...
                  <div className="card">
                    ...
                    <div className="card-body">
                      <p className="text-muted">
                        Registro completo de exámenes y consultas
                      </p>
                      <div>
                              ...
                              <div className="d-flex gap-2 align-items-end mb-3">
                                ...
                                <div className="align-self-end">
                                  <button
                                    className="btn btn-outline-primary"
                                    onClick={() => {
                                      /* filters applied by derived list below */
                                    }}
                                  >
                                    Filtrar
                                  </button>
                                ...
                                <table className="table table-hover">
                                  ...
                                  <tbody>
                                    {(() => {
                                      ...
                                      let filtered = Array.isArray(citas)
                                        ? citas.slice()
                                        : [];
                                      if (desde) {
                                        filtered = filtered.filter(
                ...
                {editEdadLocal !== null && editEdadLocal >= 18 && (
                  <div className="col-md-6 mb-3">
                    <label className="form-label">
                      DUI <small className="text-muted">(########-#)</small>
                    </label>
                    <input
                      name="dui"
                      className="form-control"
